...
const appNodes = {
	...
	// Wishes container (for falling wishes) and banner
	wishesContainer: '.wishes-container'
};
...
function renderApp(state) {
	...
	// some pages (headers) may not have SVG nodes; guard
...
function getConfigFromDOM() {
	return {
		quality: appNodes.quality ? appNodes.quality.value : String(QUALITY_NORMAL),
		shell: appNodes.shellType ? appNodes.shellType.value : 'Random',
		size: appNodes.shellSize ? appNodes.shellSize.value : '2',
		autoLaunch: appNodes.autoLaunch ? appNodes.autoLaunch.checked : true,
		finale: appNodes.finaleMode ? appNodes.finaleMode.checked : false,
		skyLighting: appNodes.skyLighting ? appNodes.skyLighting.value : String(SKY_LIGHT_NORMAL),
		longExposure: appNodes.longExposure ? appNodes.longExposure.checked : false,
		hideControls: appNodes.hideControls ? appNodes.hideControls.checked : false,
		// Store value as number.
		scaleFactor: appNodes.scaleFactor ? parseFloat(appNodes.scaleFactor.value) : getDefaultScaleFactor()
	};
};
...
function init() {
	...
	// Show a styled "Happy New Year" banner at startup
	spawnBanner('HAPPY NEW YEAR!');
}
...
function startSequence() {
	...
	if (finaleSelector()) {
		seqRandomFastShell();
		// show banner more often during finales
		if (Math.random() < 0.2) spawnBanner('HAPPY NEW YEAR!');
...
function handleResize() {
	...
	if (appNodes.stageContainer) appNodes.stageContainer.style.width = containerW + 'px';
	if (appNodes.stageContainer) appNodes.stageContainer.style.height = containerH + 'px';
...
// -------------------
// Below: added wish logic integrated into updateGlobals (falling messages)
let wishTimer = 0;
// A small list of sample wishes (Vietnamese). Edit/extend as needed.
const WISHES = [
 "Chúc cả lớp mừng năm mới!",
  "An khang thịnh vượng!",
  "Sức khỏe dồi dào!",
  "Học tập tiến bộ!",
  "Vạn sự như ý!",
  "Luôn đoàn kết và vui vẻ!",
...
function spawnWish(text) {
	if (!appNodes.wishesContainer) return;
	// Limit how many wishes on screen at once
	const maxActive = 12;
	if (appNodes.wishesContainer.children.length >= maxActive) {
		// Remove the oldest one
		const first = appNodes.wishesContainer.children[0];
	...
	// random horizontal position (centered on its own width)
	// choose a percent between 8% and 92% to avoid clipping at edges
	const leftPct = 8 + Math.random() * 84;
	el.style.left = `${leftPct}%`;

	// random size (scaled by user scale factor)
	const baseSize = 14 + Math.random() * 20; // px
	const scale = scaleFactorSelector() || 1;
	el.style.fontSize = `${Math.round(baseSize * scale)}px`;

	// color / glow
	const color = Math.random() < 0.5 ? '#fff' : COLOR_CODES[Math.random() * COLOR_CODES.length | 0];
	el.style.color = color;
	el.style.textShadow = `0 0 ${6 * scale}px rgba(255,160,20,0.85), 0 0 ${12 * scale}px rgba(255,160,20,0.6)`;

	// random duration so some fall slower
	const duration = 5 + Math.random() * 6; // seconds
	el.style.animationDuration = `${duration}s`;

	// ensure straight fall (no horizontal drift)
	el.style.setProperty('--wish-drift', '0px');

	appNodes.wishesContainer.appendChild(el);
...
// New: spawn a large centered decorative banner (HAPPY NEW YEAR)
function spawnBanner(text = 'HAPPY NEW YEAR!') {
	if (!appNodes.wishesContainer) return;
	const el = document.createElement('div');
	el.className = 'hnw-banner';
	el.textContent = text;
	// scale text size with scaleFactor; larger on desktop
	const scale = scaleFactorSelector() || 1;
	const base = IS_DESKTOP ? 84 : 42;
	el.style.fontSize = `${Math.round(base * scale)}px`;
	// animation duration (in CSS we handle fade in/out) but make sure it's removed later
...
function updateGlobals(timeStep, lag) {
	...
	// Wishes: periodically spawn falling wishes while the show is running
	if (isRunning()) {
		wishTimer -= timeStep;
		if (wishTimer <= 0) {
			// Spawn a wish
			spawnWish(randomWish());
			// randomize next spawn 1.5s - 5s (ms)
			wishTimer = 1500 + Math.random() * 3500;
		}
	} else {
		// when paused, don't spawn new wishes but let existing finish
		wishTimer = Math.max(wishTimer, 500);
	}
}
...
function colorSky(speed) {
	...
	if (appNodes.canvasContainer) appNodes.canvasContainer.style.backgroundColor = `rgb(${currentSkyColor.r | 0}, ${currentSkyColor.g | 0}, ${currentSkyColor.b | 0})`;
}
...
class Shell {
	...
	burst(x, y) {
		...
		// Optional: spawn 0-2 wishes on larger/primary bursts
		if (this.comet) {
			const count = Math.random() < 0.6 ? 1 : (Math.random() < 0.2 ? 2 : 0);
			for (let i = 0; i < count; i++) {
				spawnWish(randomWish());
			}
			// small chance to show the big banner on big bursts
			if (Math.random() < 0.04) {
				spawnBanner('HAPPY NEW YEAR!');
			}
		}
